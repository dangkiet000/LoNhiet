...
void PWM_WaitBusyTimeOK(PWM_T *pwm, uint32_t u32ChannelNum)
{
  uint16_t LusTimeOut;
  volatile uint32_t *PSyncBusyReg;
  
  if(u32ChannelNum == PWM_CH0)
    PSyncBusyReg = &((pwm)->SYNCBUSY0);
  else if(u32ChannelNum == PWM_CH1)
    PSyncBusyReg = &((pwm)->SYNCBUSY1);
  else if(u32ChannelNum == PWM_CH2)
    PSyncBusyReg = &((pwm)->SYNCBUSY2);
  else if(u32ChannelNum == PWM_CH3)
    PSyncBusyReg = &((pwm)->SYNCBUSY3);
  
  LusTimeOut = 0;
  while((LusTimeOut <5000) && (*PSyncBusyReg & 0x01) != 0)
  {
    LusTimeOut++;
  }
}
