...
/**
 * @brief       PDMA channel enable
 *
 * @param[in]   u32Ch           The selected channel
 *
 * @return      None
 *
 * @details     This function enable channel PDMA operation.
 */
void PDMA_CH_Enable(uint32_t u32Ch)
{
    PDMA_T *pdma;
    pdma = (PDMA_T *)((uint32_t) PDMA0_BASE + (0x100 * u32Ch));

    pdma->CSR |= PDMA_CSR_PDMACEN_Msk;
}

/**
 * @brief       PDMA channel disable
 *
 * @param[in]   u32Ch           The selected channel
 *
 * @return      None
 *
 * @details     This function disable channel PDMA operation.
 */
void PDMA_CH_Disable(uint32_t u32Ch)
{
    PDMA_T *pdma;
    pdma = (PDMA_T *)((uint32_t) PDMA0_BASE + (0x100 * u32Ch));
